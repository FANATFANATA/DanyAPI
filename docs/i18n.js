(function () {
    ...
    var RU = {
        ...
        st_script_p1: 'Установка одной командой: <code>irm https://raw.githubusercontent.com/FANATFANATA/DanyAPI/main/docs/install.ps1 | iex</code> в PowerShell или <code>curl -fsSL https://raw.githubusercontent.com/FANATFANATA/DanyAPI/main/docs/install.sh | bash</code> на Linux/macOS - команда скачает репозиторий и запустит <code>docs/setup.py</code>. Скрипт ставит зависимости, создаёт <code>.env</code> из <code>.env.example</code>, если его нет, спрашивает учётные данные провайдеров и настройки сервера, живым запросом проверяет токены, записывает всё в <code>.env</code> и создаёт на рабочем столе ярлык <code>DanyAPI</code>. Сервер сам обновляется до последнего релиза при каждом запуске. Нажимай <code>Enter</code>, чтобы оставить текущее значение, введи <code>!clear</code>, чтобы стереть его, или <code>!reset</code>, чтобы вернуть значение по умолчанию.',
        st_script_p2: 'Требуется Python 3.10+. Файл <code>.env</code> в gitignore и никогда не попадает в репозиторий.',
        st_install_p: 'Требуется Python 3.10+.',
        st_dev: 'Для разработки (тесты + линт) установи dev-зависимости:',
        st_ds_h: 'DeepSeek',
        st_ds_p1: 'Задай пул токенов (с разных аккаунтов) через запятую:',
        ...
        st_qw_h: 'Qwen',
        st_qw_p1: 'Токены Qwen задаются так же:',
        ...
        ug_e6: 'Провайдер не сконфигурирован (нет токенов для этого провайдера) или все его аккаунты сломаны',
        ...
        in_qw_b1: 'Auth: запросы шлют токен веб-клиента как <code>Authorization: Bearer &lt;token&gt;</code> и в cookie <code>token</code>. Токен берётся из Local Storage chat.qwen.ai (вход через email/пароль DanyAPI не использует).',
