...
OPTIONAL_STEPS = {"ruff format --check", "clang-format", "clang-tidy"}
    ...
    while i < len(argv):
        ...
        if arg in ("-j", "--jobs"):
            ...
            try:
                ...
                print(
                    f"tests: {arg} expects an integer, got {argv[i + 1]!r}",
                    file=sys.stderr,
                )
    ...
    if isinstance(runner, list):
        ...
        try:
            ...
            return (
                ...
                False,
                elapsed,
                f"timed out after {timeout:.0f}s: {cmd[0]}",
                detail,
            )
        elapsed = time.monotonic() - started
...
def print_step(result: StepResult, use_color: bool) -> None:
    ...
    if output:
        print(output)
    if ok:
        status = paint("OK", GREEN, use_color)
    elif name in OPTIONAL_STEPS:
        status = paint("WARN", YELLOW, use_color)
    else:
        status = paint("FAIL", RED, use_color)
...
def main() -> int:
    ...
    for name, ok, _elapsed, _output, _detail in results:
        if ok:
            raw = "PASS"
            mark = paint(raw.ljust(4), GREEN, use_color)
        elif name in OPTIONAL_STEPS:
            raw = "WARN"
            mark = paint(raw.ljust(4), YELLOW, use_color)
        else:
            raw = "FAIL"
    ...
    if critical_failed > 0:
        print(f"RESULT: FAILED ({critical_failed} critical check(s) failed)")
